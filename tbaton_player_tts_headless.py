#!/usr/bin/env python3
"""
Headless exporter (no live preview, no afplay):
- captions follow video frame time with "lock while TTS would play"
- builds silent captioned video
- mixes TTS at exact trigger times
- muxes into final MP4 (no original mic audio)
"""
...
def ensure_ffmpeg():
    if not shutil.which("ffmpeg"):
        raise SystemExit("[ERROR] ffmpeg not found. Install ffmpeg or `pip install imageio-ffmpeg`.")
...
def ensure_pcm_wav(src: Path, dst: Path, ar="48000", ac="1"):
    run(["ffmpeg","-y","-i",str(src),"-ar",ar,"-ac",ac,"-c:a","pcm_s16le",str(dst)], f"to PCM {src.name}")
...
def build_audio_from_events(tts_events, out_path: Path):
    ...
    for idx, (sid, t0, wav_path) in enumerate(tts_events):
        if not Path(wav_path).exists(): continue
        ...
        labels.append(f"[{lbl}]")
    if not labels:
        print("[WARN] No valid WAV inputs to mix."); return False
    ...
    try:
        ...
        print("[WARN] ffmpeg mix failed."); return False
...
def mux_video_audio(video_noaudio: Path, audio_in: Path, final_out: Path):
    ...
    try:
        ...
        print("[WARN] ffmpeg mux failed."); return False

# FFmpeg pipe writer (fallback if OpenCV writer can't open MP4)
    ...
    def __init__(self, out_path: Path, fps: float, width: int, height: int, crf="20", preset="veryfast"):
        ...
        self.proc = subprocess.Popen(
            [
                ...
                "-preset", preset,"-crf", crf,"-movflags","+faststart",
                str(out_path)
            ],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )

    def write(self, frame): self.proc.stdin.write(frame.tobytes())
    def release(self):
...
# ---------- main ----------
def main():
    ap = argparse.ArgumentParser(description="Headless caption+TTS export (preview == export logic).")
    ...
    ap.add_argument("--offset", type=float, default=0.0, help="global timing nudge seconds (+later, -earlier)")
    args = ap.parse_args()
    ...
    for s in steps:
        ...
        if d > 0.01: step_wavs[sid] = (dst, d)

    if not step_wavs:
        raise SystemExit("[ERROR] No valid TTS per-step audio found in tts_dir.")

    # open video and choose writer (OpenCV or FFmpeg pipe)
    ...
    if not writer.isOpened():
        ...
        writer = FFmpegPipeWriter(out_video_noaudio, fps, w, h)
        use_pipe = True

    # frame-driven lock-while-audio-plays simulation
    locked_sid = None
    lock_ends_at = -1.0
    ...
    while True:
        ...
        writer.write(draw_caption(frame, caption))
        frame_idx += 1

    cap.release()
    writer.release() if not use_pipe else writer.release()
    print(f"[INFO] Saved captioned (silent) video: {out_video_noaudio}")

    # build audio and mux
    tts_audio = video_path.with_name(video_path.stem + "_tts.m4a")
    if build_audio_from_events(tts_events, tts_audio):
        final_out = video_path.with_name(video_path.stem + "_captioned_TTS.mp4")
        if mux_video_audio(out_video_noaudio, tts_audio, final_out):
            print(f"✅ Final export: {final_out}")
        else:
